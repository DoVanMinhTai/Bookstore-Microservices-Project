...
export default function AddressForm({ handleSubmit, register, setValue, errors, address, isDisplay, buttonText }: AddressFormProps) {
    ...
    return (
        <div className={`shipping_address_new ${isDisplay ? '' : 'hidden'}`}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input labelText="Contact name" register={register} field="contactName" registerOptions={{ required: { value: true, message: 'This field is required' } }} defaultValue={address?.contactName} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                <OptionSelect labelText="Country" field="countryId" placeholder="Select country" options={countries} register={register} registerOptions={{ required: { value: true, message: 'Please select country' 
                ...
                <OptionSelect labelText="District" register={register} field="districtId" options={districts} placeholder="Select district" defaultValue={address?.districtId} registerOptions={{ required: { value: true, message: 'Please select district' }, onChange: (event: any) => setValue('districtName', event.target.selectedOptions[0].text) }} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <Input labelText="City" register={register} field="city" placeholder="Please skip this field if you are not in a city" defaultValue={address?.city} />
